...
int In::SomeTokens::tokensSize = NULL;
...
namespace In
{
	IN getin(std::ofstream *stream, wchar_t Infile[])
	{
		...
		while ((readSymbol = txtfile.get()) && (!txtfile.eof()))
		{
			if (StrIn.code[readSymbol] == IN::L)
			{
				...
				{
					count++; pos++;
					if (StrIn.code[readSymbol] == IN::NL)
					{
					...
					if (count == MAX_STRING - 2)
					{
						...
						while (StrIn.code[readSymbol] != IN::SEPR && StrIn.code[readSymbol] != IN::L)
						{
							readSymbol = txtfile.get();
						}
						StrIn.text[StrIn.size] = readSymbol;
			...
			{
				pos++;
				if (StrIn.text[StrIn.size - 1] == IN::PROB)
					StrIn.ignor++;
				else
					StrIn.text[StrIn.size++] = readSymbol;
				break;
			...
			{
				pos++;
				if (StrIn.code[StrIn.text[StrIn.size - 2]] == IN::SEPR && StrIn.code[StrIn.text[StrIn.size - 1]] == IN::PROB)
					StrIn.text[StrIn.size - 1] = readSymbol;
				else
					StrIn.text[StrIn.size++] = readSymbol;
				break;
	...
	void addToken(SomeTokens *tokens, char* &buf, int &posbuf, int line)
	{
		...
		strcpy_s((tokens[SomeTokens::tokensSize].token), posbuf, buf);
		tokens[SomeTokens::tokensSize].line = line;
		SomeTokens::tokensSize++;
		buf[0] = IN_CODE_NULL;
		posbuf = NULL;
	}
	...
	{
		tokens = new SomeTokens[IN_MAX_LEN_TEXT];
