...
#include "Log.h"
//синтакс
//работа конечного автомата с магазинной памятью
int FST_TRACE_n = -1;
...
namespace MFST
{
	...
	Mfst::Mfst(LexAnalaize::LEX plex, GRB::Greibach pgrebach)//(результат работы лексического анализатора, грамматика Грейбах)
	{
	...
	{
		RC_STEPEN rc = ERRORR; //код возврата = ошибка возврата
		if (lenta_position < lenta_size)
		{
			...
			else if ((st.top() == lenta[lenta_position]))//если текущий символ ленты равен вершине стека
			{
				...
				st.pop();//извлекаем вершину стека
	...
	char* Mfst::getDiagnosis(short n, char*buf)//формир ошибки синтаксич
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,%s", err.id, lex.Lextable.table[lpos].nofl, err.message);
			printf("%d: строка %d,%s\n", err.id, lex.Lextable.table[lpos].nofl, err.message);
			rc = buf;
		};
		return rc;
	};
	void Mfst::printrules(const Log::LOG &log)//отладка: вывести правила вывода
	{
		*log.stream << endl;
		*log.stream << endl;
