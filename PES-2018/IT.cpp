...
namespace IT
{
	...
	void Add(IdTable &idtable, Entry entry)
	{
		...
		idtable.table[idtable.size].value.vBOOL = TI_BOOL_DEFAULT; // false
		idtable.size++;
	}
	...
	void SetValue(IdTable &idtable, int indTI, char* token) // установка значения переменной
	{
		...
		else if (idtable.table[indTI].iddatatype == BOOL) {
			if (token == "true") {
				idtable.table[indTI].value.vBOOL = true;
			}
			else
				idtable.table[indTI].value.vBOOL = false;
		}
