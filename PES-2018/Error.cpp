...
namespace Error
{
	ERROR errors[ERROR_MAX_ENTRY] =
	{
		...
		ERROR_ENTRY(104, "Превышена длина входного параметра"),//newParm
		ERROR_ENTRY(105, "Превышен максимальный размер литерала"),//in
		ERROR_ENTRY(106, "Более одного сепаратора подряд"),//lexanalize
		...
		ERROR_ENTRY(116, "Попытка использования iffalse без iftrue"),
		ERROR_ENTRY(117, "Превышен размер таблицы лексем"),//lt
		ERROR_ENTRY(118, "Отрицательное значение литерала"),//lexanalize
		ERROR_ENTRY(119, "Отсутствие сепаратора для разделения конструкций"),//lexanalize
		...
		ERROR_ENTRY(408, "Попытка проведения операций с булевым типом данных"),
		...
		ERROR_ENTRY(606, "Ошибка в параметрах iftrue"),
		ERROR_ENTRY(607),
		ERROR_ENTRY(608),
		ERROR_ENTRY(609),
	...
	{
		ERROR AlsoError = GetError(id);
		AlsoError.position.cells = cells;
		AlsoError.position.line = line;
		return AlsoError;
	}
}